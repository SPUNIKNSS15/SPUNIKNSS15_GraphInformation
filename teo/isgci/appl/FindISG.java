...
public class FindISG{
    ...
    private static Vector<Family> families;
    private static Vector<Configuration> configurations;
    private static Vector<HMTGrammar> grammars;
    ...
    public static void main(String args[]) throws IOException,
        ...
        for (int i = 0; i < configurations.size(); i++) {
            graphs: for (int j = 0; j < graphs.size(); j++) {
                Configuration conf = configurations.elementAt(i);
                Graph g = graphs.elementAt(j);
    ...
    private static void readXMLFile(String file) throws Exception{
        ...
        for (SmallGraph g : smallGraphs) {
            ...
            if (g instanceof Graph) {
                ...
                families.addElement((Family)g);
                System.out.println("added Family");
            }
            else if (g instanceof Configuration) {
                configurations.addElement((Configuration)g);
        ...
        for (i=0; i<graphs.size(); i++) {
            for (j = i + 1; j < graphs.size(); j++) {
                if (graphs.elementAt(i).
                        isIsomorphic(graphs.elementAt(j)))
                    System.out.println("Mistake!!! " +
                            graphs.elementAt(i).getName() +
                            " isomorphic to " +
                            graphs.elementAt(j).getName());
            }
        }
        ...
        for (ci = 0; ci < configurations.size(); ci++) {
            Configuration c = configurations.elementAt(ci);
            ...
            contConf:   for (i=0; i<contained.size(); i++) {
                for (j=0; j<graphs.size(); j++)
                    if (((Graph)contained.elementAt(i)).
                         ...
                         */
                        c.addContains(graphs.elementAt(j));
        ...
        for (i=0; i<configurations.size(); i++) {
            for (j = i + 1; j < configurations.size(); j++) {
                if (configurations.elementAt(i).
                        isIsomorphic(configurations.elementAt(j)))
                    System.out.println("Mistake!!! " +
                            (configurations.elementAt(i)).getName() +
                            " isomorphic to " +
                            (configurations.elementAt(j)).getName());
            }
        }
        ...
        int curCnt;
        for (i=0; i<graphs.size(); i++){
            curCnt = (graphs.elementAt(i)).countNodes();
            if (curCnt>maxCnt)
                maxCnt = curCnt;
            if (curCnt < minCnt)
                minCnt = curCnt;
                ...
                if (((HMTFamily) families.elementAt(i)).getGrammar() != null) {
                    ...
                    contFHMT: for (j = 0; j < smMem.size(); j++) {
                        if (((Graph) smMem.elementAt(j)).countNodes() <= maxCnt) {
                            ...
                            (smMem.elementAt(j)).addLink(fhmt.getLink());
        ...
        for (i=0; i<families.size(); i++) {
            if ((families.elementAt(i) instanceof HMTFamily) &&
                    !(families.elementAt(i)).isPrimary()) {
                ...
                Vector<SmallGraph> compSmMem = new Vector<>();
    ...
    private static void addUSG(Graph g, Vector<Graph> graphs, String type){
        usg++;
        g.addName(type+usg);
    ...
    private static void sortNum(Vector<Graph> vec, int left, int right) {
        Graph y;
        int x;
        int i = left;
        int j = right;
